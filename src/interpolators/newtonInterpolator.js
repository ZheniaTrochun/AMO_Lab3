...
class Interpolator {
  static interpolate(startPoint, endPoint, funcStr, nodes) {
    ...
    for (let i = startPoint; i <= endPoint; i += len / splinesNum) {
      ...
      yArr.splice(yArr.length, 0, Interpolator.NewtonFromStart(i, xArrForInterpolation.length - 1, xArrForInterpolation, utils.calculate(xArrForInterpolation, funcStr, nodes), len / nodes));
    }
    console.log(yArr);
    return [xArr, yArr];
  }
